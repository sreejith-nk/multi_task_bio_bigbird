...
class Bigbird_multitask(LightningModule):
    ...
    def __init__(
        ...
        self.dise_lstm = nn.LSTM(self.model.config.hidden_size, self.model.config.hidden_size, batch_first=True, bidirectional=True)
        self.chem_lstm = nn.LSTM(self.model.config.hidden_size, self.model.config.hidden_size, batch_first=True, bidirectional=True)
        self.gene_lstm = nn.LSTM(self.model.config.hidden_size, self.model.config.hidden_size, batch_first=True, bidirectional=True)
        self.spec_lstm = nn.LSTM(self.model.config.hidden_size, self.model.config.hidden_size, batch_first=True, bidirectional=True)
        self.cellline_lstm = nn.LSTM(self.model.config.hidden_size, self.model.config.hidden_size, batch_first=True, bidirectional=True)
        self.dna_lstm = nn.LSTM(self.model.config.hidden_size, self.model.config.hidden_size, batch_first=True, bidirectional=True)
        self.rna_lstm = nn.LSTM(self.model.config.hidden_size, self.model.config.hidden_size, batch_first=True, bidirectional=True)
        self.celltype_lstm = nn.LSTM(self.model.config.hidden_size, self.model.config.hidden_size, batch_first=True, bidirectional=True)
        self.protein_lstm = nn.LSTM(self.model.config.hidden_size, self.model.config.hidden_size, batch_first=True, bidirectional=True)
        self.drug_lstm = nn.LSTM(self.model.config.hidden_size, self.model.config.hidden_size, batch_first=True, bidirectional=True)
        ...
        self.dise_classifier = nn.Linear(2*self.model.config.hidden_size, self.hparams.num_labels) # disease
        self.chem_classifier = nn.Linear(2*self.model.config.hidden_size, self.hparams.num_labels)
        self.gene_classifier = nn.Linear(2*self.model.config.hidden_size, self.hparams.num_labels)
        self.spec_classifier = nn.Linear(2*self.model.config.hidden_size, self.hparams.num_labels)
        self.cellline_classifier = nn.Linear(2*self.model.config.hidden_size, self.hparams.num_labels)
        self.dna_classifier = nn.Linear(2*self.model.config.hidden_size, self.hparams.num_labels)
        self.rna_classifier = nn.Linear(2*self.model.config.hidden_size, self.hparams.num_labels)
        self.celltype_classifier = nn.Linear(2*self.model.config.hidden_size, self.hparams.num_labels)
        self.protein_classifier = nn.Linear(2*self.model.config.hidden_size, self.hparams.num_labels)
        self.drug_classifier = nn.Linear(2*self.model.config.hidden_size, self.hparams.num_labels)
    ...
    def forward(self, batch: Dict[str, torch.tensor]):
        ...
        dise_outputs, _ = self.dise_lstm(outputs) # disease
        chem_outputs, _ = self.chem_lstm(outputs)
        gene_outputs, _ = self.gene_lstm(outputs)
        spec_outputs, _ = self.spec_lstm(outputs)
        cellline_outputs, _ = self.cellline_lstm(outputs)
        dna_outputs, _ = self.dna_lstm(outputs)
        rna_outputs, _ = self.rna_lstm(outputs)
        celltype_outputs, _ = self.celltype_lstm(outputs)
        protein_outputs, _ = self.protein_lstm(outputs)
        drug_outputs, _ = self.drug_lstm(outputs)
