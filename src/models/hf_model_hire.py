...
class Bigbird_multitask(LightningModule):
    ...
    def __init__(
        ...
        self.chemprot_seq = nn.Linear(self.model.config.hidden_size, int(self.model.config.hidden_size/4))
        self.ddi_seq = nn.Linear(self.model.config.hidden_size, int(self.model.config.hidden_size/4))
        self.gad_seq = nn.Linear(self.model.config.hidden_size, int(self.model.config.hidden_size/4))
        ...
        self.chemprot_classifier = nn.Linear(int(self.model.config.hidden_size/4), 6)
        self.ddi_classifier = nn.Linear(int(self.model.config.hidden_size/4), 5)
        self.gad_classifier = nn.Linear(int(self.model.config.hidden_size/4), 2)
    ...
    def forward(self, batch: Dict[str, torch.tensor]):
        ...
        protein_outputs_mean = torch.mean(protein_outputs, dim=1, keepdim=False)
        chem_outputs_mean = torch.mean(chem_outputs, dim=1, keepdim=False)
        chemprot_inputs = protein_outputs_mean + chem_outputs_mean
        chemprot_outputs=F.relu(self.chemprot_seq(chemprot_inputs))
        
        drug_outputs_mean = torch.mean(drug_outputs, dim=1, keepdim=False)
        ddi_inputs = drug_outputs_mean
        ddi_outputs=F.relu(self.ddi_seq(ddi_inputs))

        disease_outputs_mean = torch.mean(dise_outputs, dim=1, keepdim=False)
        gene_outputs_mean = torch.mean(gene_outputs, dim=1, keepdim=False)
        gad_inputs = disease_outputs_mean + gene_outputs_mean
        gad_outputs=F.relu(self.gad_seq(gad_inputs))
