...
class Bigbird_multitask(LightningModule):
    ...
    def __init__(
        ...
        self.chem_seq = nn.Linear(self.model.config.hidden_size, 10)
        self.gene_seq = nn.Linear(self.model.config.hidden_size, 20)
        self.protein_seq = nn.Linear(self.model.config.hidden_size, 5)

        self.dise_classifier = nn.Linear(self.model.config.hidden_size, self.hparams.num_labels) # disease
        self.chem_classifier = nn.Linear(10, self.hparams.num_labels) # chemical
        self.gene_classifier = nn.Linear(20, self.hparams.num_labels) # gene/protein
        ...
        self.protein_classifier = nn.Linear(5, self.hparams.num_labels) # protein
        ...
        seqs = [self.chem_seq, self.gene_seq, self.protein_seq]
    ...
    def forward(self, batch: Dict[str, torch.tensor]):
        ...
        drug_logits = self.drug_classifier(outputs) # drug logit value

        pooler = self.dropout(pooler)

        chemprot_logits = self.chemprot_classifier(pooler)
        ddi_logits = self.ddi_classifier(pooler)
        gad_logits = self.gad_classifier(pooler)
